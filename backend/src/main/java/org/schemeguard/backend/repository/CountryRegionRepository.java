...
public interface CountryRegionRepository extends CrudRepository<CountryRegionNomenclator, String> {
    ...
    @Query("SELECT c FROM CountryRegionNomenclator c WHERE c.merchant_country = :country")
    Optional<CountryRegionNomenclator> getRegion(@Param("country") String country);
}
